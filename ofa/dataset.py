...
def generate_class_info(dataset_name, root=None):
    ...
    if dataset_name == 'mvtec':
        ...
        if root is not None:
            ...
            if os.path.isfile(meta_path):
                ...
                train_meta = meta.get('train', meta)
                obj_list = sorted(list(train_meta.keys()))
            else:
                obj_list = []
                ...
                if not obj_list:
                    obj_list = [f'p_id{i}' for i in range(1, 20)]
        else:
            obj_list = [f'p_id{i}' for i in range(1, 20)]
    elif dataset_name == 'wfdd':
        obj_list = ['grey_cloth', 'grid_cloth', 'pink_flower', 'yellow_cloth']
    elif dataset_name in ['fabric-mvtec', 'fabric_mvtec']:
        obj_list = ['fabric']
...
class Dataset(data.Dataset):
    def __init__(self, root, transform, target_transform, dataset_name, mode='test'):
        ...
        self.obj_list, self.class_name_map_class_id = generate_class_info(dataset_name, root=self.root)
    def __len__(self):
        return self.length
